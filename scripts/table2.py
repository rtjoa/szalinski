...
def get_ruler(nm):
    fnm = "out/aec-table2/" + nm + ".normal-ruler.json"
    with open(fnm) as f:
        js = json.load(f)
    return str(js["final_cost"])
...
def main():
    ...
    data = {}
    for n in nms:
        data.update({n: (get_scad_loc(n), get_mesh_loc(n), get_c_in(n), get_c_out(n), get_no_cad(n), get_no_inv(n), get_ruler(n))})
    res = open("out/aec-table2/table2.csv", "w")
    res.write("Id, SCAD, #Tri, c_in, c_out, No CAD, No Inv, Ruler\n")
    res.write("TackleBox" + "," +
            data["TackleBox"][0] + "," +
            data["TackleBox"][1] + "," +
            data["TackleBox"][2] + "," +
